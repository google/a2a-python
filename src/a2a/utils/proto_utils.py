...
class ToProto:
  ...
  def message(cls, message: types.Message | None) -> a2a_pb2.Message | None:
    ...
    return a2a_pb2.Message(
        ...
        role=cls.role(message.role.name),
  ...
  @classmethod
  def update_event(
      cls, event: types.Task | types.Message | types.TaskStatusUpdateEvent
  ...
  @classmethod
  def security(
      cls, security: list[dict[str, list[str]]] | None,
    ...
    for s in security:
      rval.append(
          a2a_pb2.Security(
              schemes={
                  k: a2a_pb2.StringList(list=v.list) for (k, v) in s.items()
              }
          )
      )
    return rval
  ...
  def security_schemes(
      cls, schemes: dict[str, types.SecurityScheme] | None,
  ...
  @classmethod
  def security_scheme(
      cls, scheme: types.SecurityScheme,
  ...
  def oauth2_flows(cls, flows: types.OAuthFlows) -> a2a_pb2.OAuthFlows:
    if flows.authorizationCode:
      return a2a_pb2.OAuthFlows(
          authorization_code=a2a_pb2.AuthorizationCodeAuthFlow(
    ...
    if flows.clientCredentials:
      return a2a_pb2.OAuthFlows(
          client_credentials=a2a_pb2.ClientCredentialsAuthFlow(
...
class FromProto:
  ...
  def message_send_params(
      ...
      return types.MessageSendParams(
          configuration=cls.message_send_configuration(
  ...
  def task_id_params(
      ...
      if isinstance(request, a2a_pb2.GetTaskPushNotificationRequest):
          m = re.match(_TASK_PUSH_NOTIFICATION_NAME_MATCH, task.name)
          if not m:
              raise ServerError(
                  error=types.InvalidParamsError(
                      message=f'No task for {task.name}'
                  )
              )
          return types.TaskIdParams(id = m.group(1))
      m = re.match(_TASK_NAME_MATCH, task.name)
      if not m:
          raise ServerError(
  ...
  @classmethod
  def task_push_notification_config(
      cls, request: a2a_pb2.CreateTaskPushNotificationRequest,
  ...
  def task_query_params(
      cls, request: a2a_pb2.GetTaskRequest,
      ...
      return types.TaskQueryParams(
          historyLength=request.history_length if request.history_length else None,
          id=m.group(1),
          metadata=None,
      )
  ...
  def agent_card(
    ...
    return types.AgentCard(
        ...
        provider=cls.provider(card.provider),
        security=cls.security(card.security),
        securitySchemes=cls.security_schemes(card.security_schemes),
        skills=[cls.skill(x) for x in card.skills] if card.skills else [],
        url=card.url,
        version=card.version,
  ...
  @classmethod
  def security(
      cls, security: list[a2a_pb2.Security] | None,
  ) -> list[dict[str, list[str]]] | None:
    if not security:
      return None
    rval: list[dict[str, list[str]]] = []
    for s in security:
      rval.append({k: list(v.list) for (k, v) in s.items()})
    return rval

  @classmethod
  def security_schemes(
      cls, schemes: dict[str, a2a_pb2.SecurityScheme] | None,
  ) -> dict[str, types.SecurityScheme] | None:
    if not schemes:
      return None
    return {k: cls.security_scheme(v) for (k, v) in schemes.items()}
  ...
  @classmethod
  def security_scheme(
      cls, scheme: a2a_pb2.SecurityScheme,
  ) -> types.SecurityScheme:
    if scheme.HasApiKeySecurityScheme():
    ...
    if scheme.HasHttpAuthSecurityScheme():
      return types.SecurityScheme(root=types.HTTPAuthSecurityScheme(
    ...
    if scheme.HasOauth2SecurityScheme():
    ...
    return types.SecurityScheme(root=types.OpenIdConnectSecurityScheme(

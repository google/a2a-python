...
def create_sample_message(content="test message", msg_id="msg1", role=Role.user):
    return Message(
        messageId=msg_id,
        role=role,
...
def create_sample_status_update(task_id="task1", status_state=TaskState.working, context_id="ctx1"):
...
class TestResultAggregator(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_consume_and_emit(self):
        ...
        event2 = create_sample_task(task_id="task_event", status_state=TaskState.working)
        event3 = create_sample_status_update(task_id="task_event", status_state=TaskState.completed)
        ...
        self.mock_event_consumer.consume_all.return_value = mock_consume_generator()

        # To store yielded events
        yielded_events = []
        async for event in self.aggregator.consume_and_emit(self.mock_event_consumer):
    ...
    async def test_consume_all_other_event_types(self):
        ...
        status_update_event = create_sample_status_update(task_id="task_other_event", status_state=TaskState.completed)
        final_task_state = create_sample_task(task_id="task_other_event", status_state=TaskState.completed)
    ...
    async def test_consume_all_event_consumer_exception(self):
        ...
        self.mock_event_consumer.consume_all = AsyncMock() # Re-mock to make it an async generator that raises
        ...
        self.mock_event_consumer.consume_all = MagicMock(return_value=raiser_gen())
    ...
    async def test_consume_and_break_on_auth_required_task_event(self, mock_create_task: MagicMock):
        auth_task = create_sample_task(task_id="auth_task", status_state=TaskState.auth_required)
        ...
        self.mock_event_consumer.consume_all.return_value = mock_consume_generator()
        self.mock_task_manager.get_task.return_value = auth_task # current_result after auth_task processing
        ...
        self.assertIsInstance(self.aggregator._continue_consuming.call_args[0][0], AsyncIterator)
    ...
    @patch("asyncio.create_task")
    async def test_consume_and_break_on_auth_required_status_update_event(self, mock_create_task: MagicMock):
        auth_status_update = create_sample_status_update(task_id="auth_status_task", status_state=TaskState.auth_required)
        ...
        self.mock_task_manager.get_task.return_value = current_task_state_after_update
        self.aggregator._continue_consuming = AsyncMock()
        ...
        self.assertTrue(interrupted)
        self.mock_task_manager.process.assert_called_once_with(auth_status_update)
    ...
    async def test_consume_and_break_completes_normally(self):
        ...
        final_task_state = create_sample_task("normal_task", status_state=TaskState.completed)
        ...
        self.mock_event_consumer.consume_all.return_value = mock_consume_generator()
        self.mock_task_manager.get_task.return_value = final_task_state # For the end of stream
    ...
    async def test_consume_and_break_event_consumer_exception(self):
        ...
        async def raiser_gen_interrupt():
            ...
            raise TestInterruptException("Consumer error during interrupt check")
        self.mock_event_consumer.consume_all = MagicMock(return_value=raiser_gen_interrupt())
    ...
    async def test_continue_consuming_processes_remaining_events(self, mock_create_task: MagicMock):
        ...
        auth_event = create_sample_task("task_auth_for_continue", status_state=TaskState.auth_required)
        event_after_auth1 = create_sample_message("after auth 1", msg_id="cont1")
        ...
        self.mock_event_consumer.consume_all.return_value = initial_consume_generator()
        self.mock_task_manager.get_task.return_value = auth_event # Task state at interrupt

        # Call the main method that triggers _continue_consuming via create_task
        _, _ = await self.aggregator.consume_and_break_on_interrupt(self.mock_event_consumer)

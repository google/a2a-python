...
from a2a.types import (
    A2AError,
    JSONRPCError,
    TaskNotFoundError, # Example A2AError
    InvalidParamsError, # Example JSONRPCError (though A2AError can wrap it)
    InternalError, # Example JSONRPCError
...
class TestResponseHelpers(unittest.TestCase):
    def test_build_error_response_with_a2a_error(self):
        ...
        response_wrapper = build_error_response(request_id, a2a_error, GetTaskResponse)
        ...
        self.assertEqual(response_wrapper.root.error, specific_error) # build_error_response unwraps A2AError

    def test_build_error_response_with_jsonrpc_error(self):
        request_id = 123
        json_rpc_error = InvalidParamsError(message="Custom invalid params") # This is a specific error, not A2AError wrapped
        response_wrapper = build_error_response(request_id, json_rpc_error, GetTaskResponse)
        self.assertIsInstance(response_wrapper, GetTaskResponse)
        self.assertIsInstance(response_wrapper.root, JSONRPCErrorResponse)
        self.assertEqual(response_wrapper.root.id, request_id)
        self.assertEqual(response_wrapper.root.error, json_rpc_error) # No .root access for json_rpc_error
    ...
    def test_build_error_response_with_a2a_wrapping_jsonrpc_error(self):
        ...
        a2a_error_wrapping = A2AError(root=specific_jsonrpc_error) # Correctly wrap
        response_wrapper = build_error_response(request_id, a2a_error_wrapping, GetTaskResponse)
        self.assertIsInstance(response_wrapper, GetTaskResponse)
        self.assertIsInstance(response_wrapper.root, JSONRPCErrorResponse)
        self.assertEqual(response_wrapper.root.id, request_id)
    ...
    def test_build_error_response_with_request_id_string(self):
        ...
        response_wrapper = build_error_response(request_id, error, GetTaskResponse)
        self.assertIsInstance(response_wrapper.root, JSONRPCErrorResponse)
        self.assertEqual(response_wrapper.root.id, request_id)
    ...
    @patch(
        "a2a.server.request_handlers.response_helpers.build_error_response"
    )
    def test_prepare_response_object_with_a2a_error_instance(self, mock_build_error):
        request_id = "req_a2a_err"
        specific_error = TaskNotFoundError()
        a2a_error_instance = A2AError(root=specific_error) # Correctly wrapped A2AError
        ...
        mock_wrapped_error_response = GetTaskResponse(
            root=JSONRPCErrorResponse(id=request_id, error=specific_error, jsonrpc="2.0")
        )
        ...
        mock_build_error.assert_called_once_with(request_id, a2a_error_instance, GetTaskResponse)
    ...
    def test_prepare_response_object_with_jsonrpcerror_base_instance(
        ...
        json_rpc_base_error = JSONRPCError(code=-32000, message="Generic JSONRPC error")

        mock_wrapped_error_response = GetTaskResponse(
            root=JSONRPCErrorResponse(id=request_id, error=json_rpc_base_error, jsonrpc="2.0")
        )
        mock_build_error.return_value = mock_wrapped_error_response
        ...
        mock_build_error.assert_called_once_with(request_id, json_rpc_base_error, GetTaskResponse)
        self.assertEqual(response_wrapper, mock_wrapped_error_response)
    ...
    def test_prepare_response_object_specific_error_model_as_unexpected(
        ...
        generated_error_wrapper = A2AError(root=InvalidAgentResponseError(message='Agent returned invalid type response for this method'))
        ...
        mock_final_wrapped_response = GetTaskResponse(
            root=JSONRPCErrorResponse(id=request_id, error=generated_error_wrapper.root, jsonrpc="2.0")
        )

...
from unittest.mock import patch, AsyncMock, MagicMock # For patching sys.version_info and AsyncMock, added MagicMock

from a2a.server.events.event_queue import EventQueue, DEFAULT_MAX_QUEUE_SIZE
...
@pytest.mark.asyncio
async def test_enqueue_event_propagates_to_children(event_queue: EventQueue) -> None:
...
async def test_enqueue_event_when_closed(event_queue: EventQueue) -> None:
    ...
    child_queue = event_queue.tap() # Tap after close might be weird, but let's see
    ...
    await child_queue.close() # ensure child is also seen as closed for this test's purpose
    with pytest.raises(asyncio.QueueEmpty):
        await child_queue.dequeue_event(no_wait=True)


@pytest.mark.asyncio
async def test_dequeue_event_closed_and_empty_no_wait(event_queue: EventQueue) -> None:
...
@pytest.mark.asyncio
async def test_dequeue_event_closed_and_empty_waits_then_raises(event_queue: EventQueue) -> None:
    """Test dequeue_event raises QueueEmpty eventually when closed, empty, and no_wait=False."""
    await event_queue.close()
    assert event_queue.is_closed()
    with pytest.raises(asyncio.QueueEmpty): # Should still raise QueueEmpty as per current implementation
...
@pytest.mark.asyncio
@patch('asyncio.wait') # To monitor calls to asyncio.wait for older Python versions
@patch('asyncio.create_task') # To monitor calls to asyncio.create_task for older Python versions
async def test_close_sets_flag_and_handles_internal_queue_old_python(
    mock_create_task: MagicMock, mock_asyncio_wait: AsyncMock, event_queue: EventQueue
) -> None:
...
async def test_close_idempotent(event_queue: EventQueue) -> None:
    ...
    with patch('sys.version_info', (3, 12, 0)): # Test with older version logic first
        event_queue.queue.join = AsyncMock()
        await event_queue.close()
        assert event_queue.is_closed() is True

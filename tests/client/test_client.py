...
from httpx_sse import EventSource, ServerSentEvent, SSEError # Added SSEError
...
from a2a.types import (
    ...
    PushNotificationConfig,  # Added PushNotificationConfig
)
...
class TestA2ACardResolver:
    ...
    @pytest.mark.asyncio
    async def test_init_parameters_stored_correctly(self, mock_httpx_client: AsyncMock):
        base_url = 'http://example.com'
    ...
    async def test_init_strips_slashes(self, mock_httpx_client: AsyncMock):
        ...
        assert resolver.base_url == 'http://example.com' # Trailing slash stripped
...
class TestA2AClient:
    ...
    async def test_send_message_streaming_http_kwargs_passed(
        ...
        custom_kwargs = {'headers': {'X-Custom-Header': 'TestValue'}, 'timeout': 60}

        # Setup mock_aconnect_sse to behave minimally
        mock_event_source = AsyncMock(spec=EventSource)
        mock_event_source.aiter_sse.return_value = async_iterable_from_list([]) # No events needed for this test
        mock_aconnect_sse.return_value.__aenter__.return_value = mock_event_source

        async for _ in client.send_message_streaming(request=request, http_kwargs=custom_kwargs):
        ...
        assert called_kwargs['timeout'] == custom_kwargs['timeout'] # Ensure custom timeout is used
    ...
    async def test_send_message_streaming_sse_error_handling(
        ...
        request = SendStreamingMessageRequest(
            id="sse_err_req",
            params=MessageSendParams(message=create_text_message_object(content='SSE error test'))
        )

        # Configure the mock aconnect_sse to raise SSEError when aiter_sse is called
        mock_event_source = AsyncMock(spec=EventSource)
        mock_event_source.aiter_sse.side_effect = SSEError("Simulated SSE protocol error")
        mock_aconnect_sse.return_value.__aenter__.return_value = mock_event_source
    ...
    async def test_send_message_streaming_json_decode_error_handling(
        ...
        request = SendStreamingMessageRequest(
            id="json_err_req",
            params=MessageSendParams(message=create_text_message_object(content='JSON error test'))
        )
        ...
        mock_event_source.aiter_sse.return_value = async_iterable_from_list([malformed_sse_event])
        mock_aconnect_sse.return_value.__aenter__.return_value = mock_event_source
        ...
        assert "Expecting value: line 1 column 1 (char 0)" in str(exc_info.value) # Example of JSONDecodeError message
    ...
    async def test_send_message_streaming_httpx_request_error_handling(
        ...
        request = SendStreamingMessageRequest(
            id="httpx_err_req",
            params=MessageSendParams(message=create_text_message_object(content='httpx error test'))
        )
        ...
        mock_event_source = AsyncMock(spec=EventSource)
        mock_event_source.aiter_sse.side_effect = httpx.RequestError("Simulated network error", request=MagicMock())
        mock_aconnect_sse.return_value.__aenter__.return_value = mock_event_source
    ...
    async def test_set_task_callback_success(
        ...
        push_config_payload = PushNotificationConfig(url="https://callback.example.com/taskupdate")
        # Correctly create the TaskPushNotificationConfig (outer model)
        params_model = TaskPushNotificationConfig(taskId=task_id_val, pushNotificationConfig=push_config_payload)

        # request.id will be generated by the client method if not provided
        request = SetTaskPushNotificationConfigRequest(id="", params=params_model) # Test ID auto-generation
        ...
        with patch.object(client, '_send_request', new_callable=AsyncMock) as mock_send_req, \
             patch('a2a.client.client.uuid4', return_value=MagicMock(hex='testuuid')) as mock_uuid:

            # Capture the generated ID for assertion
            generated_id = str(mock_uuid.return_value)
            rpc_response_payload['id'] = generated_id # Ensure mock response uses the generated ID
            ...
            assert sent_json_payload['method'] == 'tasks/pushNotificationConfig/set'
            assert sent_json_payload['params'] == params_model.model_dump(mode='json', exclude_none=True)
            ...
            assert response.root.result.model_dump(mode='json', exclude_none=True) == \
    ...
    async def test_set_task_callback_error_response(
        ...
        params_model = TaskPushNotificationConfig(
            taskId="task_err_cb",
            pushNotificationConfig=push_config_payload
        )
        request = SetTaskPushNotificationConfigRequest(id=req_id, params=params_model)
        ...
        with patch.object(client, '_send_request', new_callable=AsyncMock) as mock_send_req:
            ...
            assert isinstance(response, SetTaskPushNotificationConfigResponse)
            assert isinstance(response.root, JSONRPCErrorResponse)
            assert response.root.error.model_dump(mode='json', exclude_none=True) == \
    ...
    async def test_set_task_callback_http_kwargs_passed(
        ...
        params_model = TaskPushNotificationConfig(
            taskId="task_cb_kwargs",
            pushNotificationConfig=push_config_payload
        )
        request = SetTaskPushNotificationConfigRequest(id="cb_kwargs_req", params=params_model)
        custom_kwargs = {'headers': {'X-Callback-Token': 'secret'}}

        # Minimal successful response
        rpc_response_payload: dict[str, Any] = {
            'id': "cb_kwargs_req", 'jsonrpc': '2.0',
            'result': params_model.model_dump(mode='json')
        }

        with patch.object(client, '_send_request', new_callable=AsyncMock) as mock_send_req:
            mock_send_req.return_value = rpc_response_payload
            await client.set_task_callback(request=request, http_kwargs=custom_kwargs)

            mock_send_req.assert_called_once()
            called_args, _ = mock_send_req.call_args # Correctly unpack args
            assert called_args[1] == custom_kwargs # http_kwargs is the second positional arg
    ...
    async def test_get_task_callback_success(
        ...
        params_model = TaskIdParams(id=task_id_val) # Params for get is just TaskIdParams

        request = GetTaskPushNotificationConfigRequest(id="", params=params_model) # ID is empty string for auto-generation test
        ...
        rpc_response_payload: dict[str, Any] = {
            'id': ANY,
            'jsonrpc': '2.0',
            'result': expected_callback_config.model_dump(mode='json', exclude_none=True),
        }

        with patch.object(client, '_send_request', new_callable=AsyncMock) as mock_send_req, \
             patch('a2a.client.client.uuid4', return_value=MagicMock(hex='testgetuuid')) as mock_uuid:
            ...
            assert sent_json_payload['id'] == generated_id
            assert sent_json_payload['method'] == 'tasks/pushNotificationConfig/get'
            assert sent_json_payload['params'] == params_model.model_dump(mode='json', exclude_none=True)
            ...
            assert response.root.id == generated_id
            assert response.root.result.model_dump(mode='json', exclude_none=True) == \
                   expected_callback_config.model_dump(mode='json', exclude_none=True)
    ...
    async def test_get_task_callback_error_response(
        ...
        request = GetTaskPushNotificationConfigRequest(id=req_id, params=params_model)
        error_details = TaskNotCancelableError(message="Cannot get callback for uncancelable task") # Example error
    ...
    async def test_get_task_callback_http_kwargs_passed(
        ...
        request = GetTaskPushNotificationConfigRequest(id="get_cb_kwargs_req", params=params_model)
        custom_kwargs = {'headers': {'X-Tenant-ID': 'tenant-x'}}

        # Correctly create the nested PushNotificationConfig
        push_config_payload_for_expected = PushNotificationConfig(url="https://getkwargs.com")
        expected_callback_config = TaskPushNotificationConfig(
            taskId="task_get_cb_kwargs",
            pushNotificationConfig=push_config_payload_for_expected
        )
        rpc_response_payload: dict[str, Any] = {
            'id': "get_cb_kwargs_req", 'jsonrpc': '2.0',
            'result': expected_callback_config.model_dump(mode='json')
        }

        with patch.object(client, '_send_request', new_callable=AsyncMock) as mock_send_req:
            mock_send_req.return_value = rpc_response_payload
            await client.get_task_callback(request=request, http_kwargs=custom_kwargs)
